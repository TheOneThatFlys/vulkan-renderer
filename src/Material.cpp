...
Material::Material(const VulkanEngine* engine, const Texture *base, const Texture *metallicRoughness, const Texture *ao, const Texture *normal)
    : m_base(base)
    , m_metallicRoughness(metallicRoughness)
    , m_ao(ao)
    , m_normal(normal)
{
    m_descriptorSet = engine->getRenderer()->getPipeline()->createDescriptorSet(MATERIAL_SET_NUMBER);
    ...
    engine->getDevice().updateDescriptorSets(writeDescriptors, nullptr);
}
